...
interface CourseDetailsProps {
  ...
  isEnrolled: boolean;
  onEnrollIndividual: (courseId: string, finalAmount: number, couponCode?: string) => void;
}
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {sections.map((section, idx) => (
            <div key={section.id} className="border border-[#E5ECE7] bg-white rounded-xl overflow-hidden shadow-sm">
              <button
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <strong className="text-sm sm:text-base font-bold text-[#17221B] truncate">
                    {section.title}
                  </strong>
                </div>
                  ...
                  {section.lessons.map((lesson) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <span className={\`text-xs sm:text-sm font-semibold truncate \${canLearn || lesson.isPreview ? 'text-[#17221B]' : 'text-[#66736B]'}\`}>
                          {lesson.title}
                        </span>
                      </div>
