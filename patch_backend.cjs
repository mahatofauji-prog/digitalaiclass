...
const slugRoute = `app.get('/api/courses/:slug', (req: Request, res: Response) => {
...
const updatedSlugRoute = `app.get('/api/courses/:slug', (req: Request, res: Response) => {
  const { slug } = req.params;
  const courses = DB.getCourses();
  const course = courses.find(c => c.slug === slug);
...
code = code.replace(slugRoute, updatedSlugRoute);

// Also add /api/courses/id/:id and /api/lessons/:lessonId/stream and /api/admin/courses/:id/curriculum
...
// Inject before app.listen or at the end of routes
code = code.replace('// Dashboard API', customEndpoints + '// Dashboard API');
